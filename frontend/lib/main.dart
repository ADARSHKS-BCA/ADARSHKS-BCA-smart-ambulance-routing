...
class _AuthScreenState extends State<AuthScreen> with SingleTickerProviderStateMixin {
  ...
  String? _verifiedLicenseNumber;
  String? _verifiedExpiry;
  ...
  void _submitSignUp() {
    ...
    final isLicenseValid = !_isDriver || _verificationStatus == 'VALID';

    if (isFirstNameValid && isSurnameValid && isAgeValid && isPasswordValid && isConfirmPasswordValid) {
      if (!isLicenseValid) {
        _triggerToast(
          _verificationStatus == null 
              ? 'Please scan and verify your driving license.' 
              : 'Invalid driving license verification status.',
      ...
      Timer(const Duration(seconds: 2), () {
        setState(() {
          ...
          _licenseBytes = null;
          _isVerifying = false;
          _verifiedLicenseNumber = null;
          _verifiedExpiry = null;
          _verificationStatus = null;
        });
      });
    } else {
  ...
  Future<void> _pickLicenseImage(ImageSource source) async {
    ...
    try {
      ...
      if (image != null) {
        ...
        await _verifyLicense(image, bytes);
      }
    } catch (e) {
      _triggerToast(
  ...
  Future<void> _verifyLicense(XFile image, Uint8List bytes) async {
    const String apiUrl = 'http://127.0.0.1:5000/api/verify-license';
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          _verifiedLicenseNumber = data['license_number'] ?? 'UNKNOWN';
          _verifiedExpiry = data['expiry'] ?? 'UNKNOWN';
          _verificationStatus = data['status'] ?? 'ERROR';
          _isVerifying = false;
        });
        
        if (_verificationStatus == 'VALID') {
          _triggerToast(
          ...
          _triggerToast(
            'License verification failed: $_verificationStatus',
            const Color(0xFFE74C3C),
        ...
        _triggerToast(
          errorData['error'] ?? 'Server error during verification.',
          const Color(0xFFE74C3C),
          Icons.error_outline,
        );
      }
      ...
      _triggerToast(
        'Connection to server failed. Ensure Flask is running.',
        const Color(0xFFE74C3C),
        Icons.error_outline,
      );
    }
      ...
      children: [
        Text(
          ...
          style: GoogleFonts.spaceGrotesk(
            ...
            color: const Color(0xFF0A3D5C).withOpacity(0.5),
            letterSpacing: 0.8,
          ),
        ),
        ...
        Container(
          decoration: BoxDecoration(
            ...
            border: Border.all(color: const Color(0xFF0A3D5C).withOpacity(0.1)),
          ),
  ...
  Widget _buildRoleButton({
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isActive ? const Color(0xFF0A3D5C) : Colors.transparent,
          borderRadius: BorderRadius.circular(10),
          boxShadow: isActive
              ? [
                  BoxShadow(
                    color: const Color(0xFF0A3D5C).withOpacity(0.2),
                    blurRadius: 6,
        ...
        child: Row(
          ...
          children: [
            Icon(
              icon,
              color: isActive ? Colors.white : const Color(0xFF0A3D5C).withOpacity(0.6),
              size: 18,
            ),
            ...
            Text(
              title,
              style: GoogleFonts.spaceGrotesk(
                color: isActive ? Colors.white : const Color(0xFF0A3D5C).withOpacity(0.8),
                fontSize: 13,
                fontWeight: FontWeight.bold,
              ),
            ),
  ...
  Widget _buildLicenseVerificationCard() {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: _verificationStatus == 'VALID'
              ? const Color(0xFF2ECC71).withOpacity(0.3)
              : _verificationStatus == 'EXPIRED' || _verificationStatus == 'INVALID_FORMAT'
      ...
      child: Column(
        ...
        children: [
          ...
          GestureDetector(
            onTap: _isVerifying ? null : () => _pickLicenseImage(ImageSource.camera),
            child: MouseRegion(
                    ...
                    : Column(
                        ...
                        children: [
                          Icon(
                            ...
                            color: const Color(0xFF0A3D5C).withOpacity(0.3),
                            size: 40,
                          ),
                          const SizedBox(height: 8),
                          Text(
            ...
            children: [
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: _isVerifying ? null : () => _pickLicenseImage(ImageSource.camera),
                  icon: const Icon(Icons.camera_alt, size: 16),
                  label: const Text('Take Photo'),
                  style: OutlinedButton.styleFrom(
                    foregroundColor: const Color(0xFF0A3D5C),
                    side: BorderSide(color: const Color(0xFF0A3D5C).withOpacity(0.2)),
              ...
              Expanded(
                child: OutlinedButton.icon(
                  onPressed: _isVerifying ? null : () => _pickLicenseImage(ImageSource.gallery),
          ...
          if (_isVerifying || _verificationStatus != null) ...[
            ...
            if (_isVerifying)
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Running OCR & validating with server...',
                      style: TextStyle(
                        fontSize: 12,
              ...
              _buildVerificationResultWidget(),
          ],
        ],
      ),
    );
  ...
  Widget _buildVerificationResultWidget() {
    final bool isValid = _verificationStatus == 'VALID';
    final Color primaryColor = isValid 
        ? const Color(0xFF2ECC71) 
        : _verificationStatus == 'VALID_MISSING_EXPIRY'
            ? const Color(0xFFF1C40F)
            : const Color(0xFFE74C3C);

    final IconData icon = isValid
        ? Icons.check_circle
        : _verificationStatus == 'VALID_MISSING_EXPIRY'
            ? Icons.info
            : Icons.cancel;

    final String statusLabel = _verificationStatus == 'VALID'
        ? 'VERIFIED VALID'
        : _verificationStatus == 'EXPIRED'
            ? 'EXPIRED LICENSE'
            : _verificationStatus == 'INVALID_FORMAT'
                ? 'INVALID FORMAT'
                : _verificationStatus == 'VALID_MISSING_EXPIRY'
                    ? 'VALID (NO EXPIRY)'
                    : 'VERIFICATION ERROR';
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_verifiedLicenseNumber != null && _verifiedLicenseNumber!.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(bottom: 6.0),
              child: RichText(
                text: TextSpan(
                  style: const TextStyle(fontSize: 13, color: Color(0xFF0A3D5C)),
                  children: [
                    const TextSpan(
                      text: 'License Number: ',
                      style: TextStyle(fontWeight: FontWeight.bold),
                    ),
                    TextSpan(text: _verifiedLicenseNumber),
                  ],
                ),
              ),
            ),
          if (_verifiedExpiry != null && _verifiedExpiry!.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(bottom: 6.0),
              child: RichText(
                text: TextSpan(
                  ...
                  children: [
                    ...
                    TextSpan(
                      text: _verifiedExpiry,
                      style: TextStyle(
                        color: _verificationStatus == 'EXPIRED' ? const Color(0xFFE74C3C) : null,
                        fontWeight: _verificationStatus == 'EXPIRED' ? FontWeight.bold : null,
                      ),
                    ),
                  ],
                ),
  ...
  String _getInstructionTextForStatus() {
    switch (_verificationStatus) {
      case 'VALID':
        return 'Success: Your license is active and format is valid.';
      case 'EXPIRED':
        return 'Error: The license expiry date is in the past. Registration is blocked.';
      case 'INVALID_FORMAT':
        return 'Error: OCR could not verify a valid license structure. Try taking a clearer, well-lit photo.';
      case 'VALID_MISSING_EXPIRY':
        return 'Notice: License pattern matches, but the expiry year could not be read. Please review.';
      default:
        return 'Verification failed due to server error or connection timeout.';
    }
  }
